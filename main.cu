...
int main(int argc, char* argv[]) {
    ...
    if (myRank == root) {
	    ...
	    if (r == root)
	       continue;
            ncclRecv(recvbuff+(r-1)*offset, size, ncclFloat, r, comm, s);
            ncclSend(sendbuff+(r-1)*offset, size, ncclFloat, r, comm, s);
        }
    } else {
    ...
    // NCCLCHECK(ncclGroupStart());
    // if (myRank == root) {
    //     ncclRecv(recvbuff, size, ncclFloat, 0, comm, s);
    ...
    //     ncclSend(sendbuff, size, ncclFloat, root, comm, s);
    //     ncclRecv(recvbuff, size, ncclFloat, root, comm, s);
	// ncclBcast(sendbuff, size, ncclFloat, 0, comm, s);
